...
Seven persona rules decide what an answer looks like. The first four replaced a hard 2–6-sentence cap and a blanket markup ban that between them produced one dense unbroken paragraph for every question, whatever the question was — a contact lookup and "what makes his work different from other people's?" came back the same shape and the same length, and the measured figures in the knowledge base never survived the squeeze.
...
- **Credentials are context, never headlines.** Affiliations, partnerships, awards, grants and funding get stated plainly, once, and in proportion to the question, and an answer never *opens* on one when the question asked what something is or how it works. The trigger was CoLA: "tell me about CoLA" came back leading on the Meta Project Aria partnership and hardware grant as a headline credential, which oversells the affiliation — on an academic homepage that costs credibility with exactly the readers who matter. The rule also forbids letting a credential grow in the telling: a research partnership and a hardware grant are exactly that, **not employment, not a product, not a joint publication, not an endorsement**. It is a calibration rule, not a deletion rule — it names the Meta grant, the LAK '25 Best Short Paper Award, the Novo Nordisk Foundation-funded project with Life Campus and the UCPH PhD fellowship together, and requires each to stay in the answer where it is relevant, in the knowledge base's own words.
- **Contact details are an answer, not a sign-off.** The old rule said only that meeting, collaboration and recruiting enquiries should get `zali@di.ku.dk` and the LinkedIn profile, and the model generalised it into a universal footer — nearly every reply ended "for implementation details or collaboration, email zali@di.ku.dk". The rule now gives the two cases where the contact details *are* the answer: the visitor asks how to reach him, or asks about collaboration, hiring, supervision, a position or a meeting; or the knowledge base cannot answer and the email is the genuine next step. Otherwise they do not appear at all.
...
Persona rules that work with those blocks and should stay: answer the question asked, leading with the specific fact rather than a project summary and never opening consecutive answers the same way; when the knowledge does not cover a detail, say so in one sentence and offer the nearest fact instead of reciting the intro; and the [Answer shape](#answer-shape) rules, whose specificity and contrast clauses are what actually pull these numbers into an answer while the credential-calibration clause keeps a grant or an award from displacing them at the top.
