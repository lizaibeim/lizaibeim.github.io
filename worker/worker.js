...
Architecture: data moves in three stages. Sensors and badges stream to base stations or to a central media server. Each base station runs one or more Base components — one per pipeline, so an AudioBase for speech, and its equivalents for positioning and video — which handle the signal processing and offload the heavy work to REST services, while a matching Synchronizer merges per-person results into group-level segments over MQTT and writes them to the time series database. The dashboard renders those measurement features live and builds post-time visualisations, logs and reports from the same store. The LAK 2025 paper frames the same platform as a multilayered IoT architecture over a client-server split — a Sensing Layer; a Network Layer carrying Ethernet, WiFi, MQTT and RESP; a Data Processing Layer of base stations, database servers and AI services; an Analytic Layer; and a Presentation Layer — five layers, and that paper's view of this one arrangement rather than a second design.
